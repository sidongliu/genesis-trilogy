...
yielding~\eqref{eq:anti_correlation}.
...
\end{itemize}
The numerical demonstration in Section~\ref{sec:numerical} provides preliminary evidence for this transition.
...
Theorem~\ref{thm:tradeoff} predicts that the mutual information between an agent's compressed representation and the full environmental state satisfies:
\begin{equation}
\frac{I_{\mathrm{discarded}}}{I_{\mathrm{pred}}} \gtrsim 1 - \frac{k^*}{D}
\label{eq:discard-scaling}
\end{equation}
...
Theorem~\ref{thm:delusion} predicts a critical timescale $t_{\mathrm{del}} = \Lambda^{-1} \ln(\pi / 4\theta_0)$ beyond which a fixed-frame agent's gradient updates become anti-correlated with the true optimum.
