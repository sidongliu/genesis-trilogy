...
Paper A & DOI: 10.5281/zenodo.18361706 & Emergent Geometry \\
Paper B & DOI: 10.5281/zenodo.18367060 & Accessibility \& Stability \\
Essay C & DOI: 10.5281/zenodo.18374805 & Causation \& Agency \\
Paper D & DOI: 10.5281/zenodo.18388881 & Gravitational Phenomena \\
Paper E & DOI: 10.5281/zenodo.18400065 & Measurement \\
Paper F & DOI: 10.5281/zenodo.18400425 & Temporal Asymmetry \\
Paper G & DOI: 10.5281/zenodo.18402907 & Structural Limits \\
Postscript & DOI: 10.5281/zenodo.18407367 & Closure of Structure \\[1em]
\multicolumn{3}{l}{\textbf{Part~II --- Q-RAIF}} \\[0.5em]
Paper A & DOI: 10.5281/zenodo.18525876 & Lorentzian Metrics \\
Paper B & DOI: 10.5281/zenodo.18525890 & Thermodynamic Stability \\
Paper C & DOI: 10.5281/zenodo.18528934 & Realizability Bridge \\[1em]
\multicolumn{3}{l}{\textbf{Part~III --- T-DOME}} \\[0.5em]
...
This volume collects the thirteen papers and postscript comprising the Genesis Trilogy---a programme that investigates the minimal structural conditions under which physical descriptions, persistent agents, and self-referential observers can arise within quantum theory.

The programme is organised in three parts:

\begin{enumerate}
\item \textbf{Part~I: HAFF} (Papers A--G, Essay~C, Postscript).
  The Holographic Alaya-Field Framework treats the tensor factorisation of Hilbert space as a derived, rather than assumed, structure.  Starting from a single global quantum state and coarse-grained observable algebras, it argues that:
  (a)~emergent geometry, locality, and topology follow from algebraic accessibility;
  (b)~gravitational phenomena can be understood as evolution \emph{of} the accessible algebra (formulated as a structural conjecture, not a derivation);
...
\item \textbf{Part~II: Q-RAIF} (Papers A--C).
  The Quantum Reference Algebra for Information Flow asks what algebraic structure a persistent subsystem must possess if it is to survive within the geometry established by Part~I.  The three papers argue that:
...
We show that non-commuting coarse-graining maps generically induce distinct emergent spacetimes from the same underlying state, conditional on the applicability of a geometric reconstruction dictionary (e.g., Ryu--Takayanagi in holographic settings).
...
When a geometric reconstruction dictionary applies, this inequivalence reflects a genuine multiplicity of effective structures at the emergent level, not merely a coordinate ambiguity.
...
In the algebraic QFT setting, this corresponds to the cyclic vector of a Type III$_1$ von Neumann algebra \cite{Haag1996}, representing a holistically entangled substrate containing the localized ``seeds'' (eigenmodes) of all possible emergent geometries. The present paper primarily works in finite-dimensional (Type I) settings where standard von Neumann entropy is well-defined; the extension to Type III factors would require reformulation in terms of relative entropy.
...
Let $\mathcal{H}_{\text{total}}$ be a finite-dimensional Hilbert space with $\dim \mathcal{H}_{\text{total}} = d$, and let
\[
...
\label{H-rem:A-infinite-dim}
Theorem~\ref{H-thm:A-nofact} is stated for finite-dimensional $\mathcal{H}_{\text{total}}$, where the Haar measure and Page's concentration inequality are well-defined.
For separable infinite-dimensional Hilbert spaces, the measure-theoretic argument does not directly apply (Haar measure on infinite-dimensional unit spheres is not normalizable).
However, the conclusion is expected to hold \emph{a fortiori}: in the algebraic setting, generic states on type~III$_1$ factors (the relevant class for quantum field theory) have infinite entanglement entropy across any bipartition, making the non-factorizability even stronger.
The algebraic core of the argument (Step~2, Zanardi et al.) extends without modification to the infinite-dimensional case.
\end{remark}
...
If $\mathbf{c}_1 \not\sim \mathbf{c}_2$
(i.e., they are not related by unitary equivalence),
then:
\begin{enumerate}
...
Then the induced connectivity functions $\mu^{(1)}$ and $\mu^{(2)}$ define distinct entanglement structures.
If, in addition, a geometric reconstruction theorem applies (e.g., the Ryu--Takayanagi dictionary in holographic settings), the resulting emergent geometries are generically inequivalent.
\end{theorem}
...
\label{H-ex:A-fermion}
Consider $N$ free fermions on a one-dimensional lattice with a spectral gap (e.g., from dimerization) and ground state $|\Psi_0\rangle$.
...
The area-law spatial graph and the volume-law momentum graph have qualitatively different entanglement scaling (constant vs.\ $N\ln N$), and therefore define distinct entanglement structures.
Under any geometric reconstruction dictionary that maps entropy profiles to distance structures, $\mathbf{c}_x$ and $\mathbf{c}_p$ yield inequivalent emergent geometries from the same underlying state $|\Psi_0\rangle$.
\end{example}
...
\end{tikzpicture}
\caption{Schematic of the HAFF structure. A single global state $|\Psi_{\text{vac}}\rangle$ (Alaya) projects into generically distinct emergent spacetimes ($M_1, M_2$) depending on the choice of observable algebra ($\mathcal{A}_1, \mathcal{A}_2$). This illustrates that geometry is observer-dependent in the fundamental sense.}
...
\item When a geometric reconstruction theorem applies, these inequivalent coarse-grainings generically give rise to distinct emergent geometries (Theorem~\ref{H-thm:A-geom}).
\end{enumerate}
...
\item \textbf{Geometry dependence} (Theorem 3): When a geometric reconstruction dictionary applies, inequivalent coarse-graining structures generically induce inequivalent geometric structures from the same underlying state, which may include distinct topological features in certain cases.
\end{enumerate}
...
Expectation values of operators in $\mathcal{A}$ remain approximately invariant under physically motivated dynamical maps $\mathcal{E}$ (representing decoherence, RG flow, or measurement-induced back-action; here $\mathcal{E}$ acts in the Heisenberg picture on observables):
\begin{equation}
...
Following Zurek's quantum Darwinism framework \cite{Zurek2009}, stable subalgebras are those whose information is redundantly encoded in environmental degrees of freedom. Formally, the subalgebra $\mathcal{A}$ should approximately commute with the environmental algebra $\mathcal{A}_E$ generated by accessible environmental observables (in operator norm):
\begin{equation}
...
\item[\textup{(U2)}] \textbf{Modular stability}: The \emph{ambient} modular automorphism group $\sigma_t^{\omega,\mathrm{tot}}$ of $(\mathcal{B}(\mathcal{H}_{\mathrm{total}}), \omega)$ preserves $\mathcal{A}_{\mathbf{c}}$: $\sigma_t^{\omega,\mathrm{tot}}(\mathcal{A}_{\mathbf{c}}) = \mathcal{A}_{\mathbf{c}}$ for all $t \in \mathbb{R}$.
\item[\textup{(U3)}] \textbf{Maximality}: $\mathcal{A}_{\mathbf{c}}$ is the maximal subalgebra of $\mathcal{B}(\mathcal{H}_{\mathrm{total}})$ satisfying \textup{(U1)--(U2)} together with the three accessibility criteria of Section~\ref{H-sec:B-accessibility} (dynamical invariance, environmental redundancy, non-scrambling).
\end{enumerate}
...
\begin{remark}[Proof status and relation to Paper~D]
\label{H-rem:B-uniqueness-scope}
The proof sketch above suppresses two gaps that are made explicit in the refined version of this conjecture (Conjecture~\ref{H-conj:D-uniqueness}, Paper~D): (G1)~faithfulness of the join algebra, and (G2)~the lattice-theoretic dichotomy.
Readers should consult Conjecture~\ref{H-conj:D-uniqueness} for the current state of the proof.
...
\begin{remark}
We emphasize that this definition captures \emph{predictive asymmetry} (analogous to Granger causality) rather than interventionist causation in the sense of Pearl~\cite{Pearl2009} or Woodward. Common causes can produce asymmetric mutual information without direct causal influence.
\end{remark}
...
The asymmetry in Definition~\ref{H-def:C-causal-asymmetry} can be grounded in thermodynamic considerations.
Systems approaching equilibrium exhibit increasing entropy, inducing a preferred temporal direction even when microscopic dynamics are time-symmetric~\cite{Wallace2012Time}.

\begin{sloppypar}
Crucially, this thermodynamic arrow is \emph{context-dependent}: it depends on which macrostates are accessible, which in turn depends on the coarse-graining. Different accessible algebras may therefore induce different thermodynamic gradients and hence different effective causal structures.
\end{sloppypar}
...
\rho_{AB}(t) &\approx \frac{1}{2}\left(|00\rangle\langle 00| + e^{-\Gamma_A t}|00\rangle\langle 11| + e^{-\Gamma_A t}|11\rangle\langle 00| + |11\rangle\langle 11|\right),
\end{align}
...
Crucially, if we had chosen a different coarse-graining---say, tracing over qubit degrees of freedom and retaining environmental observables---the causal arrow might, depending on the specific bath model, reverse or disappear. The asymmetry is \emph{real} (measurable within $\mathcal{A}_{\mathbf{c}}$) but \emph{context-dependent} (algebra-relative).
...
Yog\=ac\=ara (``Yoga practice'') is a Mah\=ay\=ana Buddhist philosophical school emphasizing the role of consciousness (\emph{vij\~n\=ana}) in constituting experienced reality. Central to Yog\=ac\=ara is the concept of \textbf{\=alaya-vij\~n\=ana} (Skt., ``storehouse consciousness''), a foundational stratum of mind that stores karmic seeds (\emph{b\={\i}ja}) conditioning future experience \cite{Asanga_Mahayana,Vasubandhu_Trimsika}.

A structural parallel may be noted: \=alaya-vij\~n\=ana functions as a holistic substrate from which individuated mental states emerge, analogous to how subsystem structures emerge from coarse-graining a global quantum state \cite{Lusthaus2002,Waldron2003}.
...
\hline
\=Alaya-vij\~n\=ana (storehouse consciousness) & Global quantum state $|\Psi_U\rangle$ \\
B\={\i}ja (karmic seeds) & Eigenmodes of accessible algebras \\
Prav\d{r}tti-vij\~n\=ana (active consciousness) & Effective subsystem $\rho_{\text{eff}}$ \\
...
The structural parallel between \=alaya-vij\~n\=ana and the global quantum state $|\Psi_U\rangle$ does \textbf{not} imply:
\begin{itemize}
...
The analogy is \emph{purely structural}: both frameworks describe how individuated entities emerge from holistic substrates via coarse-graining or cognitive filtering. Yog\=ac\=ara's substrate is explicitly mental (\emph{vij\~n\=ana}, consciousness); HAFF's substrate is explicitly physical (quantum state in Hilbert space).
...
\item \=Alaya-vij\~n\=ana is explicitly described as a form of consciousness, while $|\Psi_U\rangle$ is not imbued with phenomenological properties.
...
The M\=adhyamaka (``Middle Way'') tradition, founded by N\=ag\=arjuna (c.\ 2nd century CE), articulates \textbf{\'{s}\=unyat\=a} (Skt., ``emptiness'') as the absence of \textbf{svabh\=ava} (Skt., ``intrinsic nature'' or ``own-being'') \cite{Nagarjuna_MMK_Garfield1995}. 

This concept bears structural resemblance to the notion of ``emptiness'' developed in Part III (\S\ref{H-sec:C-III.3}), where it denoted absence of intrinsic being without entailing non-existence. We emphasize that this parallel concerns the \emph{structural form} of the claim---denial of substance while affirming functional reality---not historical influence or causal connection \cite{Garfield2002,Siderits2007}.
...
A structural reading: entities lacking intrinsic being (empty) can nonetheless participate in stable relational networks (dependent arising). This maps onto the framework developed in \S\ref{H-sec:C-III.4}: subsystems are empty (coarse-graining-dependent, lacking intrinsic factorization) yet existent (exhibiting objective entanglement structure).

\paragraph{Recursive emptiness and second-order structure.}
A subtle question arises: in M\=adhyamaka, emptiness applies universally, including to emptiness itself---``emptiness is empty'' (\emph{\'{s}\=unyat\=a-\'{s}\=unyat\=a}). Does HAFF's relational existence admit similar recursive application?
...
M\=adhyamaka \'{s}\=unyat\=a is deployed soteriologically---to undermine attachment to fixed views and facilitate liberation. HAFF's ``emptiness'' is a technical descriptor of relational structure, with no soteriological function. The similarity is formal, not practical or existential.

\subsection{Karma and Constraint Propagation}
\label{H-sec:C-IV.4}

Buddhist karma doctrine holds that intentional actions leave traces (\emph{sa\.msk\=ara}, ``formations'') that condition future experience. In Yog\=ac\=ara, these traces are stored in \=alaya-vij\~n\=ana as \emph{v\=asan\=a} (``karmic impressions'') \cite{Waldron2003}.
...
This is consonant with the account of memory developed in \S\ref{H-sec:C-I.5}: causal traces are not storage of intrinsic properties but maintenance of relational structure across time.

\paragraph{Critical limitation.}
Buddhist karma is inherently normative: actions are classified as wholesome (\emph{ku\'{s}ala}) or unwholesome (\emph{aku\'{s}ala}) based on ethical criteria and soteriological consequences. HAFF's constraint propagation is descriptive, lacking normative content. The formal parallel does not extend to ethical or soteriological dimensions.

\subsection{Limits and Divergences}
\label{H-sec:C-IV.5}

Having noted possible parallels, we now emphasize substantive divergences---areas where Buddhist frameworks and HAFF diverge structurally, methodologically, or conceptually. This section is weighted equally to \S\ref{H-sec:C-IV.2}--\ref{H-sec:C-IV.4} to prevent over-interpreting formal similarities.

\subsubsection{Phenomenological vs.\ Structural Orientation}

Buddhist philosophy is fundamentally concerned with first-person experience and liberation from suffering (\emph{du\d{h}kha}). Yog\=ac\=ara and M\=adhyamaka analyze consciousness, perception, and mental afflictions (\emph{kle\'{s}a}) as prerequisites for soteriological transformation.
...
\textbf{Consequence}: Any parallel between \=alaya-vij\~n\=ana and $|\Psi_U\rangle$ cannot extend to phenomenological dimensions. HAFF does not explain consciousness, nor does it claim that quantum states possess mental properties.
...
We conclude Part IV by reaffirming interpretive humility. The parallels identified---between accessible algebras and \=alaya-vij\~n\=ana, between emptiness and \'{s}\=unyat\=a, between constraint propagation and karma---are \emph{suggestive but inconclusive}.
...
We note that this argument is \emph{kinematic} (all matter inhabits the same geometry) rather than \emph{dynamical} (all matter follows geodesics of that geometry). The full dynamical content of the Weak Equivalence Principle---that test masses follow geodesics of the emergent metric---requires additional input beyond the algebraic framework presented here.

\begin{remark}[Dark Sector as Algebraic Inaccessibility]
This perspective suggests a natural interpretation of ``dark'' degrees of freedom. Matter that does not couple to our accessible algebra $\mathcal{A}_{\mathbf{c}}$---while potentially present in the global state $|\Psi_U\rangle$---would be operationally invisible except through its gravitational effects on the geometry induced by $\mathcal{A}_{\mathbf{c}}$---a possibility that requires extending the framework to allow degrees of freedom outside $\mathcal{A}_{\mathbf{c}}$ to influence the algebra selection process, which is not currently formalized. This is speculative but structurally consistent with the framework.
\end{remark}
...
\begin{enumerate}
    ...
    \item This shift $\dot{\mathcal{A}}_{\mathbf{c}}(t)$ manifests phenomenologically as dynamical spacetime curvature---i.e., as~gravity.
\end{enumerate}
...
\begin{remark}[Scope of the distinction]
This categorical distinction is expected to be valid only as a \emph{low-energy, effective} characterization. In UV-complete theories where gauge and gravitational degrees of freedom are unified (as in string theory/M-theory dualities), the distinction between ``evolution within $\mathcal{A}_{\mathbf{c}}$'' and ``evolution of $\mathcal{A}_{\mathbf{c}}$'' may break down. The adiabatic assumption (slow algebra transitions relative to internal dynamics) is essential for the distinction to be operationally meaningful.
\end{remark}
...
To make the algebra-geometry correspondence precise, we propose that the emergent metric is \emph{related to} the \textbf{Quantum Fisher Information Metric (QFIM)}, a standard construction in quantum information geometry \cite{Petz1996,Bengtsson2006}.
...
\begin{remark}[Caveats on the QFIM identification]
Two issues must be addressed before the QFIM can serve as a literal spacetime metric:
\begin{enumerate}
    \item[(a)] The map from algebra deformations $\delta\mathcal{A}_{\mathbf{c}}$ to state deformations $\delta\rho$ (which enter the QFIM) must be specified explicitly; this map depends on the state--algebra relationship and is not uniquely determined by the framework.
    \item[(b)] The QFIM is positive-definite (Riemannian), while physical spacetime has Lorentzian signature. The mechanism by which a Lorentzian metric emerges from a Riemannian information-geometric construction---the ``signature problem''---remains open.
\end{enumerate}
\end{remark}

Under the hypothesis that gravitational dynamics reflects algebra evolution (Conjecture~\ref{H-conj:main}), the Einstein tensor $G_{\mu\nu}$ may be understood as describing the curvature of this information manifold. Changes in the accessible algebra along a one-parameter family $\mathcal{A}_{\mathbf{c}}(\lambda)$ with $\mathcal{A}_{\mathbf{c}}(0) = \mathcal{A}_{\mathbf{c}}$
...
\label{H-sec:D-modular}

\begin{sloppypar}
The preceding sections formulated the HAFF gravity conjecture in structural and information-geometric terms.
The accessibility criteria (Definition~\ref{H-def:D-accessible}) were stated in approximate, operational language.
Here we translate these criteria into Tomita--Takesaki modular theory, state a uniqueness conjecture with a proof sketch, verify the construction in two examples, and connect to linearized gravity via the entanglement first law.
\end{sloppypar}


\subsection{Preliminaries}
\label{H-sec:D-modular-prelim}
...
\label{H-sec:D-modular-def}

We now reformulate the physical accessibility criteria (Definition~\ref{H-def:D-accessible}) in algebraic terms.
...
\label{H-def:D-modular-accessible}
...
\begin{remark}[Modular theory in finite dimensions]
For finite-dimensional $\mathcal{H}_U$, $\mathcal{B}(\mathcal{H}_U)$ is a Type~I factor and the modular flow reduces to Hamiltonian evolution: $\sigma_t^{\omega,\mathrm{tot}}(X) = \rho^{it} X \rho^{-it}$ where $\rho$ is the density matrix of $\omega$. Condition (U2) then becomes: $\mathcal{A}_{\mathbf{c}}$ is invariant under conjugation by $\rho^{it}$. The full power of Tomita--Takesaki theory becomes essential only in the infinite-dimensional/QFT setting (Type~III factors), as illustrated by the Rindler wedge example. The qubit chain example is a pedagogical illustration in the Type~I setting.
\end{remark}

\begin{remark}[Relation to physical criteria]
\label{H-rem:D-translation}
...
\hline
(P1) Dyn.\ invariance & (U3) $\mathcal{E}_t^*(\mathcal{A}_{\mathbf{c}}) \subset \mathcal{A}_{\mathbf{c}}$ & Decoherence invariance \\
(P2) Redundancy & (U1) Faithfulness & Faithful state $\Leftrightarrow$ no lost info \\
(P3) Non-scrambling & (U2) Modular stability & $\sigma_t^{\omega,\mathrm{tot}}$ preserves $\mathcal{A}_{\mathbf{c}}$ \\
\hline
\end{tabular}
}
\end{center}
...
The modular automorphism group $\sigma_t^\omega$ is the canonical ``internal time evolution'' of the algebra with respect to the state $\omega$ (Theorem~\ref{H-thm:D-KMS-unique}).
...
\subsection{Uniqueness Conjecture}
\label{H-sec:D-modular-uniqueness}

We conjecture that the modular accessible algebra, if it exists, is unique up to unitary equivalence under a suitable ergodicity condition.
This refines Conjecture~\ref{H-conj:B-uniqueness} (Paper~B) by making the ergodicity assumption and proof gaps explicit.
A complete proof remains open; we present the conjecture and a proof sketch that identifies the key steps and the gaps that remain to be closed.

\begin{conjecture}[Uniqueness of the modular accessible algebra]
\label{H-conj:D-uniqueness}
...
\medskip
\noindent\textbf{(E)} \textit{The joint action of the ambient modular flow $\sigma_t^{\omega,\mathrm{tot}}$ and the Heisenberg-picture decoherence semigroup $\mathcal{E}_t^*$ is ergodic on $\mathcal{B}(\mathcal{H}_U)$: the only operator invariant under both is a scalar multiple of the identity.}
\medskip
...
We flag at each step the assumptions that go beyond (E).

\medskip
\noindent\textbf{Step 1: Uniqueness of modular flow (rigorous).}
By Theorem~\ref{H-thm:D-KMS-unique}, for any faithful normal state $\omega$ on a von Neumann algebra $\mathcal{M}$, the modular automorphism group $\sigma_t^\omega$ is the \emph{unique} one-parameter automorphism group satisfying the KMS condition at $\beta = 1$.
...
\medskip
\noindent\textbf{Step 2: Lattice of invariant subalgebras (partially rigorous).}
...
Together, $\mathcal{A}_{\mathbf{c}}$ lies in the lattice $\mathfrak{L}$ of von Neumann subalgebras of $\mathcal{B}(\mathcal{H}_U)$ that are simultaneously invariant under $\sigma_t^{\omega,\mathrm{tot}}$ and $\mathcal{E}_t^*$, and on which $\omega$ is faithful.

\textit{Invariance is preserved under joins.}
If $\mathcal{A}_1, \mathcal{A}_2 \in \mathfrak{L}$, then $\sigma_t^{\omega,\mathrm{tot}}(\mathcal{A}_1 \vee \mathcal{A}_2) = \mathcal{A}_1 \vee \mathcal{A}_2$ (automorphisms respect the lattice of von Neumann subalgebras).
Similarly, $\mathcal{E}_t^*$ preserves $\mathcal{A}_1 \vee \mathcal{A}_2$.
However, $\omega$ need not be faithful on the join: this is a genuine obstruction.

\medskip
\noindent\textbf{Step 3: Maximality argument (gap identified).}
The intended conclusion is: if $\mathcal{A}_{\mathbf{c}}$ and $\mathcal{A}'$ are both maximal elements of $\mathfrak{L}$, then $\mathcal{A}_{\mathbf{c}} = \mathcal{A}'$ up to unitary equivalence.
The natural strategy is to show that $\mathcal{A}_{\mathbf{c}} \vee \mathcal{A}'$ either belongs to $\mathfrak{L}$ (contradicting maximality unless $\mathcal{A}_{\mathbf{c}} = \mathcal{A}'$) or violates one of the conditions.

\textit{What is established}: Invariance of the join under both dynamics (Step 2).

\textit{What remains open}: Two gaps must be closed to complete the proof:
\begin{enumerate}
    \item[(G1)] \textbf{Faithfulness of the join}: Ergodicity (E) constrains the \emph{fixed-point} algebra of the joint dynamics, but does not directly control faithfulness of $\omega$ on the \emph{invariant} subalgebras.
    A stronger assumption---e.g., that $\mathcal{E}_t$ is a \emph{Schwarz map} with no non-trivial decoherence-free subalgebra---would close this gap by ensuring that any proper extension leaves the domain of faithfulness.
    \item[(G2)] \textbf{Dichotomy for the join}: The argument that ``$\mathcal{A}_{\mathbf{c}} \vee \mathcal{A}'$ violates (U1) or (U2) implies $\mathcal{A}_{\mathbf{c}} \subset \mathcal{A}'$'' requires an additional lattice-theoretic step: that there is no ``third possibility'' where the join fails (U3) while neither algebra contains the other.
    This holds if $\mathfrak{L}$ is a \emph{modular lattice} (in the lattice-theoretic sense), but this has not been verified for the relevant class of von Neumann subalgebras.
    The lattice of von Neumann subalgebras of $\mathcal{B}(\mathcal{H})$ is known to be non-modular in general~\cite{GeKadison1996}; closing this gap likely requires restricting to specific structural classes (e.g., subfactors of finite Jones index).
\end{enumerate}

We expect that both gaps can be closed for physically relevant models (finite-dimensional or hyperfinite type~III$_1$ factors), but a general proof is not yet available.
The residual unitary freedom (between different faithful normal representations of the same abstract algebra) would be absorbed by the standard form~\cite{Haagerup1975}.
\end{proof}

\begin{remark}[On the ergodicity assumption and the gaps]
\label{H-rem:D-ergodicity}
...
Closing gap (G1) is the more tractable of the two: for finite-dimensional systems, faithfulness is equivalent to the state having full support, and joins of supported subalgebras remain supported.
Gap (G2) is more subtle and may require restricting to specific classes of decoherence semigroups.
We flag this as a problem for future work.
\end{remark}
...
This illustrates both the content of (U2)---it imposes a genuine compatibility condition between the algebra and the state---and its limitations: the qubit chain example is a partial verification, with (U1) and (U3) satisfied exactly and (U2) satisfied approximately in the strong-dephasing regime. Strictly, $\mathcal{A}_{\mathrm{diag}}$ is a modular accessible algebra for the dephasing-dominated steady state $\rho_{\mathrm{ss}}$, not for an arbitrary thermal state of the full Hamiltonian.
...
In the language of half-sided modular inclusions (Theorem~\ref{H-thm:D-Wiesbrock}), the perturbation can be described by
\begin{equation}
\mathcal{A}_{\mathbf{c}}' = e^{-i\delta a\, P} \mathcal{A}_{\mathbf{c}}\, e^{i\delta a\, P}, \qquad \delta a \ll 1,
\end{equation}
yielding
\begin{equation}
\label{H-eq:D-deltaK-translation}
\delta K_{\mathrm{mod}} = -i\delta a\, [P, K_{\mathrm{mod}}] + O(\delta a^2).
\end{equation}
We emphasize that this formula applies specifically to HSMI-type algebra deformations (null translations of wedge algebras in the Rindler setting). For generic algebra perturbations, the modular Hamiltonian response requires the full Connes cocycle/Araki perturbation theory, which is substantially more involved. The derivation chain below therefore applies rigorously only in settings where the algebra perturbation has HSMI structure.
\end{enumerate}

In the general case (combined perturbation):
\begin{equation}
\label{H-eq:D-deltaK-total}
...
\label{H-thm:D-linearized-Faulkner}
...
\subsubsection{Conditional result within the HAFF framework}

\begin{corollary}[Linearized gravity from accessible algebra perturbation]
\label{H-cor:D-linearized}
...
\end{enumerate}
Then a perturbation of the accessible algebra along a one-parameter family $\mathcal{A}_{\mathbf{c}}(\lambda)$ with $\mathcal{A}_{\mathbf{c}}(0) = \mathcal{A}_{\mathbf{c}}$ induces a perturbation of the modular Hamiltonian $\delta K_{\mathrm{mod}}$, and the entanglement first law
\begin{equation}
...
\begin{remark}
The new content of this result is limited to identifying the input of the Faulkner et al.\ derivation (modular Hamiltonian perturbation) with the output of HAFF algebra evolution ($\delta\mathcal{A}_{\mathbf{c}} \to \delta K_{\mathrm{mod}}$ via the HSMI formula). The gravitational content is entirely provided by the holographic assumptions (H1)--(H3).
\end{remark}
...
Wiesbrock's theorem (Theorem~\ref{H-thm:D-Wiesbrock}) recovers translations from half-sided modular inclusions; a sufficient net of such inclusions reconstructs the full Poincar\'{e} group.
...
In Corollary~\ref{H-cor:D-linearized}, we adopt resolution~(C).
...
\begin{enumerate}
    ...
    \item \textbf{No unconditional derivation of Einstein equations:} The linearized result (Section~\ref{H-sec:D-linearized}) is conditional on holographic assumptions (H1)--(H3); we do not derive general relativity from first principles.
...
The algebraic foundation established in Section~\ref{H-sec:D-modular} sharpens the open problems facing the HAFF gravity program. The following are the immediate targets for subsequent work.

\subsection{Linearized Einstein Equations without Holography}

Corollary~\ref{H-cor:D-linearized} derives the linearized Einstein equations conditionally, assuming the holographic correspondence (H1)--(H3). A fully general derivation from algebraic perturbation theory would require establishing a ``bulk reconstruction'' from the modular data of $\mathcal{A}_{\mathbf{c}}$ and its perturbations, using Wiesbrock-type inclusions (Theorem~\ref{H-thm:D-Wiesbrock}) to define spacetime translations algebraically. This would remove the dependence on AdS/CFT and extend the result to non-holographic settings.

\subsection{Moduli Space when Ergodicity Fails}

The uniqueness conjecture (Conjecture~\ref{H-conj:D-uniqueness}) requires the ergodicity assumption~(E). When (E) fails---in systems with spontaneous symmetry breaking, topological order, or phase coexistence---the space of accessible algebras acquires a non-trivial moduli space (Remark~\ref{H-rem:D-ergodicity}). Characterizing this moduli space, and determining whether it carries a natural metric (e.g., the Fisher information metric on modular Hamiltonians) that encodes the geometry of the space of effective descriptions, is an important structural problem.

\subsection{Computing $\delta K_{\mathrm{mod}}$ beyond Linear Order}

The connection to linearized gravity (Section~\ref{H-sec:D-linearized}) uses only the first-order perturbation of the modular Hamiltonian. Computing $\delta K_{\mathrm{mod}}$ explicitly for perturbations of the accessible algebra in specific models, beyond the linear order, is essential for accessing nonlinear gravitational dynamics. The second-order correction may contain information about the gravitational coupling constant $G_N$ and matter content.
...
The algebraic foundation developed in Section~\ref{H-sec:D-modular} partially formalizes this framework, translating structural intuitions into mathematically precise (though not yet fully proven) statements. By translating the three physical accessibility criteria into Tomita--Takesaki modular theory---faithfulness (U1), modular stability (U2), and maximality (U3)---we conjectured that the accessible algebra is unique up to unitary equivalence under ergodicity assumptions (Conjecture~\ref{H-conj:D-uniqueness}), identified the specific gaps (G1--G2) that remain to be closed, and verified the construction in both the Rindler wedge (reproducing Bisognano--Wichmann and Unruh physics) and a qubit chain with decoherence (recovering the pointer basis algebra). Most significantly, we established the derivation chain
\begin{equation}
...
\end{equation}
showing that perturbations of the accessible algebra, via modular Hamiltonian perturbation and the entanglement first law, yield the linearized Einstein equations in holographic settings (Corollary~\ref{H-cor:D-linearized}). This makes precise the HAFF gravity conjecture at the linearized level: in holographic settings satisfying (H1)--(H3), the adiabatic flow of the accessible algebra \emph{is} linearized gravitational dynamics, conditionally derived rather than postulated.
Removing the holographic assumptions (H1)--(H3) remains an open problem (Section~\ref{H-sec:D-open}).
...
Conserved quantities that commute with $H_{\mathrm{int}}$ (e.g., total energy or charge in number-conserving systems) would fail this criterion. In such cases, the constraint should be interpreted as requiring that the observable can be \emph{correlated} with external degrees of freedom, whether through direct interaction coupling or through pre-existing correlations and boundary conditions.
\end{remark}
...
Strictly, the set of operators individually satisfying all three constraints need not form a $*$-algebra (algebraic closure under sums and products is not guaranteed). We define $\mathcal{A}_{\mathbf{c}}$ as the von Neumann algebra \emph{generated by} the operators satisfying the three accessibility constraints. The generating operators satisfy the constraints individually; the algebraic closure step is an idealization whose validity in specific models remains to be verified.
...
\begin{quote}
\textbf{Redundant environmental encoding explains the \emph{effective} definiteness of measurement outcomes within each decoherence branch, but does not by itself select a unique outcome from the global superposition.}
\end{quote}

The relationship between redundancy and absolute definiteness depends on one's interpretation of quantum mechanics. The global quantum state remains in superposition; what becomes definite is the content of redundant records, which all agree on the same eigenvalue within each branch.
...
Among the deepest puzzles in physics is the origin of temporal asymmetry. The fundamental laws of physics---Newtonian mechanics, electromagnetism, quantum mechanics---are time-reversal invariant, with the notable exception of the weak interaction (CP violation in the CKM matrix, established experimentally in B-meson systems). However, this microscopic T-violation is quantitatively insufficient to explain the macroscopic arrow of time. Yet our experience of the world is profoundly asymmetric: eggs break but do not unbreak; we remember the past but not the future; causes precede effects.
...
We note that this argument is a specific instance of the general statistical-mechanical arrow of time (Boltzmann's combinatorial argument), applied to the redundancy index rather than entropy. The derivation presupposes a dynamical evolution parameter; what is derived is the \emph{arrow} (directionality), not time itself.
...
This definition assumes that algebraic inclusion and redundancy increase are consistent---i.e., that enlarging the algebra does not decrease the redundancy of the original operators. This monotonicity property holds for quantum Darwinism in the Markov limit but has not been established in general.
...
\label{H-prop:retro}
Retrocausal trajectories are not excluded by principle, but are exponentially suppressed and macroscopically atypical.
\end{proposition}
...
\begin{enumerate}
    ...
    \item \textbf{Retrocausation non-generic}: Retrocausal trajectories are not excluded by principle, but are exponentially suppressed and macroscopically atypical.
...
\begin{lemma}[Associativity Requirement]
\label{Q-A-lem:assoc}
...
The following is not a derivation but a statement of the minimal algebraic requirement, with physical motivation.
...
We note that every quantum state space carries a Fisher--Rao metric; the constraint here is not that a metric \emph{exists} (which is trivially satisfied) but that it is \emph{canonically encoded} in the algebraic structure of the generating space, requiring no external specification.
\end{proof}
...
For error correction with an external metric, the correction protocol must \emph{independently specify} $G$ at each step.
Since a general symmetric bilinear form on $n$ generators has $n(n+1)/2$ independent parameters, the information cost of specifying an external metric grows quadratically with the number of generators.
In a Clifford algebra, by contrast, the metric is encoded in the anticommutation relations themselves: once the algebra is fixed, no additional metric specification is needed.

This distinction becomes physically relevant when the error correction must operate within the thermodynamic bounds of T-DOME (Paper~I): the entropy production rate for maintaining NESS is bounded, and the additional overhead of specifying an external metric at each correction cycle may become prohibitive for large $n$.
We emphasize that this is a heuristic argument for the efficiency of built-in metrics, not a formal complexity-theoretic result; making it rigorous would require specifying an explicit computational model for the error-correction protocol.
\end{remark}

\subsection{Constraint III: Causal Channel Encoding}

\begin{proposition}[Indefinite Signature within the Clifford Framework]
\label{Q-A-lem:signature}
Within a Clifford algebraic framework, a non-trivial causal structure---distinguishing time-like from space-like separation---requires the bilinear form to have indefinite signature $(p,q)$ with $p \geq 1$, $q \geq 1$.
\end{proposition}
...
By contraposition from Step~2: if $q$ is positive-definite, the automorphism group is compact and the causal structure is trivial.
For the even subalgebra to contain both compact generators (spatial rotations) and non-compact generators (boosts)---enabling an invariant causal cone in the representation space---the quadratic form must have indefinite signature: $\mathrm{sig}(q) = (p,r)$ with $p, r \geq 1$.
The resulting group $\mathrm{Spin}(p,r)$ is non-compact and admits the required invariant cone structure~\cite{Haag1996}.
...
The present argument establishes that, within the Clifford algebraic setting, indefiniteness is a \emph{necessary condition}---under the Clifford framework adopted in Constraints~I and~II---for non-trivial causal structure.
Alternative algebraic frameworks (e.g., non-associative or higher-categorical) might in principle encode causal distinctions differently; the claim here is conditional on the Clifford structure established by Constraints~I and~II.
\end{proof}
...
The marks ``$\times$'' for von Neumann and $C^*$-algebras indicate that these categories are too \emph{general} to encode the required structure canonically, not that they are incompatible: every Clifford algebra is itself a special case of a $C^*$-algebra and (in finite dimensions) a von Neumann algebra. The point is that additional structure (the quadratic form) must be imposed externally.
...
\paragraph{Constraint III (constructive ansatz).}
The Pauli algebra alone encodes a Euclidean signature $(3,0)$.
To obtain a Lorentzian signature, we \emph{posit} a distinguished direction corresponding to the modular Hamiltonian $K$, which generates modular flow (the boundary analog of time evolution in the bulk), and \emph{assume} that it anticommutes with the spatial generators:
\begin{equation}
(iK)^2 = -\mathbf{1}, \qquad \sigma_i^2 = +\mathbf{1}, \qquad \{iK, \sigma_i\} = 0.
\end{equation}
Under these assumptions, the extended algebra is $Cl(1,3)$.
We emphasize that the anticommutation $\{iK, \sigma_i\} = 0$ is an additional structural assumption, not derived from the tensor network model; no known holographic model produces this anticommutation relation, and establishing (or refuting) it is a major open problem. It amounts to requiring that the modular direction and spatial directions jointly generate a Clifford algebra rather than a more general associative algebra.
...
\begin{proposition}[Clifford Compatibility]
\label{Q-A-prop:selection}
...
\label{Q-A-rem:scope}
Proposition~\ref{Q-A-prop:selection} is a statement about \emph{algebraic compatibility}, not physical uniqueness.
...
Lemma~\ref{Q-A-lem:assoc} (associativity as a boundary condition, substantiated by the pathologies of non-associative alternatives),
Lemma~\ref{Q-A-lem:metric} (non-degenerate bilinear form from holographic error correction, connected to the modular Hamiltonian and Fisher information),
and Lemma~\ref{Q-A-lem:signature} (indefinite signature from the algebraic structure of causal order).
...
This derivation assumes local Lorentz invariance---a condition naturally satisfied when the boundary algebra is Clifford-compatible, since $Cl(1,3)$ contains $\mathrm{Spin}(1,3)$ (the double cover of the Lorentz group) as a group within its even subalgebra $Cl^0(1,3)$.
...
\begin{remark}[Logical weight of the constraints]
Constraint~I (associativity) is a background requirement automatically satisfied by any operator algebra on a Hilbert space. Constraint~II (built-in metric) is a naturality/economy criterion rather than a strict physical necessity---any quantum system carries a Fisher--Rao metric, but requiring it to be algebraically encoded narrows the candidates. Constraint~III (indefinite signature) carries the primary discriminating power. The three constraints are therefore not logically independent: the argument reduces to one genuine physical requirement (indefinite signature within an algebraically natural metric framework) plus background assumptions.
\end{remark}
...
The interaction of a quantum system with a large environment typically leads to decoherence and thermalization~\cite{BreuerPetruccione2002,Weiss2012}.
...
The claim is structural: under the built-in metric assumption (Constraint~II), any subsystem maintaining persistence encodes its environment using a Clifford-compatible algebra.
...
We acknowledge that this constraint is automatically satisfied by operator algebras on Hilbert spaces, where composition of linear maps is inherently associative~\cite{BreuerPetruccione2002}.
...
The physical pathologies of non-associative alternatives are discussed in Remark~\ref{Q-A-rem:nonassoc} of Paper~A.
...
For a persistent subsystem to distinguish qualitatively different environmental coupling channels and implement directed error correction, the control algebra must carry a non-degenerate bilinear form.
Among encodings of the qualitative distinction between channel types, an indefinite signature provides the most parsimonious algebraic encoding that requires no external labeling.
\end{lemma}
...
A positive-definite anisotropic metric (e.g., $\mathrm{diag}(a, b, c)$ with $a \neq b \neq c > 0$) can encode quantitative differences between directions.
In principle, an external labeling scheme (e.g., a discrete index classifying channels as ``dissipative'' vs.\ ``unitary'') could supplement such a metric to encode qualitative distinctions.

However, an indefinite quadratic form $q(v) = \eta_{\mu\nu} v^\mu v^\nu$ with $\mathrm{sig}(\eta) = (p,r)$, $p,r \geq 1$, encodes the dichotomy \emph{intrinsically}: positive-norm directions ($v^2 > 0$) correspond to one class, negative-norm directions ($v^2 < 0$) to another, with the distinction built into the algebra via $v^2 = q(v)\mathbf{1}$.
No external labeling is needed.
The assignment of negative norm to dissipative directions is a structural \emph{ansatz} motivated by the algebraic analogy, not a derived result from open quantum systems theory; establishing this correspondence rigorously remains an open problem.

We therefore argue that indefinite signature is the \emph{most parsimonious} encoding of the reversible/irreversible dichotomy within an algebraic framework, though we do not claim it is the \emph{unique} possibility.
\end{proof}
...
We acknowledge that semisimple Lie algebras do carry canonical metrics (the Killing form); the advantage of the Clifford structure is specifically that it encodes a metric on the \emph{generating vector space} $V$, with potentially indefinite signature, rather than only on the Lie algebra.
...
When the subsystem must distinguish dissipative from unitary perturbations---e.g., $\gamma_\downarrow \neq 0$ (dissipative) versus Hamiltonian noise (unitary)---the control space naturally splits into sectors of different character.
An indefinite quadratic form provides a parsimonious algebraic encoding of this split, upgrading $Cl(3,0)$ to $Cl(p,q)$ with appropriate signature, though alternative encodings using positive-definite metrics supplemented by external labeling are not excluded in principle.

% ============================================================
\subsection{The Algebraic Compatibility Theorem}

\begin{proposition}[Persistence Compatibility]
\label{Q-B-prop:main}
...
\begin{corollary}
Under the built-in metric assumption (Constraint~II), any subsystem maintaining NESS for $\tau \gg \tau_{\mathrm{relax}}$ while discriminating among environmental channels encodes its boundary using a Clifford-compatible algebra.
\end{corollary}
...
\label{Q-B-rem:scope}
As with Proposition~\ref{Q-A-prop:selection} in Paper~A, the final step of the proof---invoking the universal property of $Cl(V,q)$---is a standard algebraic identity.
The substantive content resides in the constraints:
Lemma~\ref{Q-B-lem:assoc} (associativity as a boundary condition, with non-associative alternatives producing quantifiable pathologies; see Remark~\ref{Q-A-rem:nonassoc}),
and Lemma~\ref{Q-B-lem:metric} (indefinite metric from the requirement of channel discrimination in directed error correction).
The worked example in Section~\ref{Q-B-sec:toymodel} demonstrates that the constraints are simultaneously satisfiable in a concrete Lindblad model.
\end{remark}
...
\begin{itemize}
    ...
    Paper A (Chapter~\ref{Q-chap:paperA}) argues that $\mathcal{A}_{\mathrm{ext}}$ must contain $Cl(1,3)$ as its minimal Clifford-compatible subalgebra (Proposition~\ref{Q-A-prop:selection}, ``Clifford Compatibility'').
    ...
    Paper B (Chapter~\ref{Q-chap:paperB}) argues that thermodynamic persistence requires $\mathcal{O}_{\mathrm{int}} \cong Cl(V,q)$ for some $(V,q)$ (Proposition~\ref{Q-B-prop:main}, ``Persistence Compatibility'').
\end{itemize}
...
Since $\mathcal{O}_{\mathrm{int}} \cong Cl(V,q)$ by the persistence requirement (Proposition~\ref{Q-B-prop:main}), and $\phi$ is structure-preserving, $\mathcal{O}_{\mathrm{eff}}$ inherits the Clifford relations $v^2 = q(v)\mathbf{1}$~\cite{Hestenes1966}.
Moreover, by Lemma~\ref{Q-C-lem:semisimple}, if\/ $\dim V$ is even then $Cl(V,q)$ is simple and $\phi$ is necessarily injective; the image is therefore isomorphic to $Cl(V,q)$ itself, giving a genuine embedding $Cl(V,q) \hookrightarrow \mathcal{A}_{\mathrm{ext}}$.
...
\label{Q-C-rem:scope}
The logical structure of Theorem~\ref{Q-C-thm:closure} is essentially: ``a Clifford algebra, homomorphically embedded in a larger algebra, lands as a Clifford subalgebra.''
The non-trivial content resides not in this deduction but in the \emph{input conditions}:
(i)~Paper~A's argument that $\mathcal{A}_{\mathrm{ext}} \supseteq Cl(1,3)$;
(ii)~Paper~B's argument that persistence forces $\mathcal{O}_{\mathrm{int}} \cong Cl(V,q)$;
(iii)~the injectivity guarantee from semisimplicity (Lemma~\ref{Q-C-lem:semisimple}).
The theorem's role is to assemble these independently substantiated conditions into a single algebraic closure statement, not to introduce new mathematical content.
\end{remark}
...
Here ``signature incompatible with $(1,3)$'' means that the quadratic space $(V',q')$ does not embed isometrically into $(\mathbb{R}^{1,3}, \eta)$, where we use the convention that $p$ generators square to $+1$ and $q$ generators square to $-1$.
\end{corollary}
...
% PART III: T-DOME
% ============================================================================
\part[T-DOME: Observer-Memory Dynamics]{T-DOME: Thermodynamic Dynamics of\\Observer-Memory Entanglement}
\label{part:TDOME}
...
the trace distance $D(\rho_1(t), \rho_2(t))$, can
temporarily increase~\cite{BreuerLainePiilo2009}.
This is the operational signature of memory---the environment
returns previously absorbed information to the system.
\end{remark}
...
\begin{theorem}[Thermal Markovian Ceiling]
\label{T-I-thm:ceiling}
Assume the GKSL generator satisfies quantum detailed balance with
respect to a single thermal bath at inverse temperature $\beta$
(i.e., the KMS detailed-balance condition of
Lemma~\ref{T-I-lem:spohn}).
Let $\Lambda^{\mathrm{M}}$ denote the resulting GKSL
dynamics~\eqref{T-I-eq:GKSL}, coupled to the stationary thermal bath,
under a control protocol
...
\item \textbf{Dissipative structures} ($\mathcal{S} \leq 0$):
  ...
  (Schr\"odinger's sense~\cite{Schrodinger1944}.)
...
\begin{remark}[Weak-coupling caveat]
\label{T-I-rem:weak_coupling_caveat}
The identity~\eqref{T-I-eq:advantage} holds exactly under the
weak-coupling approximation ($\lambda \to 0$); for finite coupling,
$O(\lambda)$ corrections from the interaction energy
$H_{\mathrm{int}}$ should be tracked.
For autonomous evolution ($H_{\mathrm{ctrl}} = 0$), the
identity~\eqref{T-I-eq:advantage_auto} is exact.
\end{remark}
...
\label{T-I-conj:fisher_accretion}
...
We emphasize that this argument is informal: no formal connection
between the Fisher information $I_F(\theta; \rho(t))$ (about an
environmental parameter) and the quantum mutual information
$I(S{:}E)$ (between system and environment) has been established.
A rigorous derivation via the quantum Cram\'er--Rao bound remains
an open problem.
...
This monotonicity holds on timescales coarse-grained over the bath
correlation time; during non-Markovian backflow intervals, the number
of ``good'' fragments could temporarily decrease.
The inclusion
...
exactly for the chosen initial state ($r_z = 0$, equal populations),
where the population contribution to $D_{\mathrm{KL}}$ is constant
and cancels in $\Delta D_{\mathrm{KL}}$
(see, e.g.,~\cite{BreuerPetruccione2002}).
...
\label{T-I-rem:bound_tightness}
This bound is tight only for environments with long-range temporal
correlations (infinite predictive information
$I_{\mathrm{pred}}$). For short-range-correlated environments, the
mutual information between past and present is $O(1)$ (the
\emph{excess entropy}), and the memory cost is bounded rather than
divergent.
\end{remark}
...
rate $h_\mu > 0$.
Assume the $D$ environmental components carry independent
information at rate $h_\mu$ each.
...
\begin{remark}[Accounting identity]
\label{T-II-rem:accounting_identity}
We note that this result is an accounting identity under the
independence assumption: joint processing across correlated channels
could in principle reduce the total rate below $D \cdot h_\mu$. The
independence assumption is the substantive physical input.
\end{remark}
...
\label{T-II-rem:SSB_terminology}
We use ``symmetry breaking'' by analogy with condensed matter physics,
where the term has precise mathematical content (degenerate ground
states, Goldstone modes, divergent susceptibility). Here the
``symmetry'' is the gauge freedom of the Clifford algebra, and
``breaking'' it amounts to choosing a reference frame under resource
constraints. More conservative terminology---``reference frame
selection'' or ``basis choice under computational
constraints''---may be appropriate until formal SSB criteria
(Landau-type free energy with degenerate minima) are established for
this setting.
\end{remark}
...
\begin{remark}[Uniformity assumption]
\label{T-II-rem:uniformity}
The bound $I_{\mathrm{discarded}} \geq (1 - k^*/D)\,
I_{\mathrm{pred}}$ holds under the uniformity assumption (each
component carries $\sim I_{\mathrm{pred}}/D$ predictive
information). In practice, predictive information is often
concentrated in a few dominant modes; in such cases the discarded
fraction may be substantially smaller, making gauge fixing less
costly.
\end{remark}
...
Part~(b):
Decompose $\mathcal{S}_{\mathrm{full}}$ in the agent's
(rotated) coordinates as
$\mathcal{S}_{\mathrm{full}}(u,v)
= \mathcal{S}_{\mathrm{ff}}(u)
+ \mathcal{S}_{\mathrm{fb}}(u,v)
+ \mathcal{S}_{\mathrm{bb}}(v)$,
where $u \in V_{\mathrm{fg}}$, $v \in V_{\mathrm{bg}}$,
$\mathcal{S}_{\mathrm{ff}}$ depends only on foreground
controls, $\mathcal{S}_{\mathrm{bb}}$ only on background,
and $\mathcal{S}_{\mathrm{fb}}$ encodes the
foreground--background cross-coupling.
The projected kernel
$\mathcal{K}_{\mathcal{F}}
= \Pi_{\mathcal{F}}\,\mathcal{K}\,\Pi_{\mathcal{F}}$
discards $\mathcal{S}_{\mathrm{fb}}$ and
$\mathcal{S}_{\mathrm{bb}}$, so
$\mathcal{S}_{\mathrm{vis}}(u)
= \mathcal{S}_{\mathrm{ff}}(u)$.
The foreground gradients are therefore
$\nabla_u\mathcal{S}_{\mathrm{vis}}
= \nabla_u\mathcal{S}_{\mathrm{ff}}$
and
$\nabla_u\mathcal{S}_{\mathrm{full}}
= \nabla_u\mathcal{S}_{\mathrm{ff}}
+ \nabla_u\mathcal{S}_{\mathrm{fb}}$,
giving
\[
\bigl\langle
  \nabla_u\mathcal{S}_{\mathrm{vis}},\;
  \nabla_u\mathcal{S}_{\mathrm{full}}
\bigr\rangle
= \bigl|\nabla_u\mathcal{S}_{\mathrm{ff}}\bigr|^2
+ \bigl\langle
  \nabla_u\mathcal{S}_{\mathrm{ff}},\;
  \nabla_u\mathcal{S}_{\mathrm{fb}}
\bigr\rangle.
...
Under the rotating drift model, the frame rotation
by~$\theta$ induces cross-coupling that scales as
$\sin\theta\cos\theta$ (maximal at $\theta = \pi/4$),
while the direct term scales as $\cos^4\!\theta$.
The sign of $\nabla_u\mathcal{S}_{\mathrm{fb}}$
is set by the background state~$v$: since the agent
invests no control resources in $V_{\mathrm{bg}}$,
$v$~relaxes toward the uncontrolled equilibrium,
where the cross-coupling penalises
foreground-directed updates
($\langle \nabla_u\mathcal{S}_{\mathrm{ff}},\,
\nabla_u\mathcal{S}_{\mathrm{fb}}\rangle < 0$).
Beyond $t_{\mathrm{del}}$ ($\theta > \pi/4$),
the adverse coupling dominates the direct term,
yielding~\eqref{T-II-eq:anti_correlation}.

The sign of the cross-coupling term
$\nabla_u \mathcal{S}_{\mathrm{fb}}$ depends on the specific
drift model (rotating optimal frame with $\cos^2/\sin^2$
decomposition). For a general Clifford algebra, the cross terms
between foreground and background could have either sign depending
on environmental correlations. The anti-correlation result is
therefore established for the drift model of
Section~\ref{T-II-subsec:drift_model} and conjectured to hold generically
...
The results of this paper yield several quantitative predictions that are, in principle, falsifiable in computational and biological systems.
We emphasize that these predictions follow from the \emph{classical} content of T-DOME: the thermodynamic and information-theoretic bounds apply to any open system maintaining a non-equilibrium steady state under bounded computation, regardless of whether the underlying dynamics are quantum or classical.

\paragraph{Prediction 1: Attention entropy collapse under resource constraints.}
Theorem~\ref{T-II-thm:SSB} predicts that any adaptive agent operating under computational budget $\mathcal{C}_{\mathrm{budget}}$ in an environment with $D$ relevant degrees of freedom and metric entropy $h_\mu$ will exhibit a sharp symmetry-breaking transition when $\mathcal{C}_{\mathrm{budget}} < D \cdot h_\mu \cdot \tau_{\mathrm{mem}}$.
The order parameter---attention entropy $H_{\mathrm{attn}} = -\sum_i p_i \ln p_i$ over the agent's allocation of computational resources---should collapse from $\ln D$ (symmetric phase) to $\ln k^*$ (broken phase), with $k^* = \lfloor \mathcal{C}_{\mathrm{budget}} / h_\mu \rfloor$.

\textbf{Protocol}: Train reinforcement learning agents on a $D$-dimensional partially observable environment with systematically varied computational budgets (context window, hidden state dimension, or parameter count).
Measure attention entropy as a function of $\mathcal{C}_{\mathrm{budget}}/D$.
The predicted transition should be:
\begin{itemize}
\item Sharp (not gradual) in the limit $D \to \infty$
\item Located at $\mathcal{C}_{\mathrm{budget}} / (D\, h_\mu\, \tau_{\mathrm{mem}}) \approx 1$
\item Accompanied by spontaneous selection of $k^*$ ``preferred'' features
\end{itemize}
The numerical demonstration in Section~\ref{T-II-sec:numerical} provides preliminary evidence for this transition.
Extending to deep RL agents with continuous state spaces would constitute a non-trivial test.

\paragraph{Prediction 2: Information-discarded scaling law.}
Theorem~\ref{T-II-thm:tradeoff} predicts that the mutual information between an agent's compressed representation and the full environmental state satisfies:
\begin{equation}
\frac{I_{\mathrm{discarded}}}{I_{\mathrm{pred}}} \gtrsim 1 - \frac{k^*}{D}
\label{T-II-eq:discard-scaling}
\end{equation}
under the uniformity assumption.
This is a specific, quantitative prediction: an agent retaining $k^*$ out of $D$ features discards at least a fraction $1 - k^*/D$ of the total predictive information.

\textbf{Protocol}: For RL agents with tunable bottleneck width $k$, measure the mutual information $I(\hat{X}; X)$ between the compressed internal state $\hat{X}$ and the full environmental state $X$ using variational estimators~\cite{PooleOzairVanDenOord2019}.
Plot $I_{\mathrm{discarded}} / I_{\mathrm{pred}}$ against $k/D$.
The prediction is a linear lower bound with slope $-1/D$ and intercept $1$.

\paragraph{Prediction 3: Delusion trap timescale.}
Theorem~\ref{T-II-thm:delusion} predicts a critical timescale $t_{\mathrm{del}} = \Lambda^{-1} \ln(\pi / 4\theta_0)$ beyond which a fixed-frame agent's gradient updates become anti-correlated with the true optimum.

\textbf{Protocol}: Deploy a fixed-architecture RL agent in an environment with controlled rotation rate $\Lambda$ of the reward-relevant features.
Measure the cosine similarity between the agent's policy gradient $\nabla_u \mathcal{S}_{\mathrm{vis}}$ and the oracle gradient $\nabla_u \mathcal{S}_{\mathrm{full}}$ as a function of time.
The prediction:
\begin{itemize}
\item Cosine similarity $> 0$ for $t < t_{\mathrm{del}}$ (updates improve performance)
\item Cosine similarity crosses zero at $t \approx t_{\mathrm{del}}$
\item Cosine similarity $< 0$ for $t > t_{\mathrm{del}}$ (updates degrade performance)
\item The crossover timescale scales as $\Lambda^{-1} \ln(1/\theta_0)$
\end{itemize}
This is directly testable in non-stationary RL benchmarks with controlled distributional shift.

\paragraph{Prediction 4: Biological information bottleneck.}
The ego-entropy tradeoff is not specific to artificial agents.
Any biological system maintaining homeostasis under bounded sensory processing capacity should exhibit the same $k^*/D$ scaling.
In bacterial chemotaxis, the bacterium processes $D \sim 5$--$10$ chemical gradients through a signaling pathway with $k^* \sim 2$--$3$ effective internal states (CheY-P phosphorylation levels).
The prediction is:
\begin{equation}
\frac{I(\text{internal state};\, \text{chemical environment})}{I(\text{chemical environment};\, \text{survival})} \lesssim \frac{k^*}{D} \approx 0.2\text{--}0.6
\label{T-II-eq:bio-bound}
\end{equation}
Single-cell measurements of chemotactic mutual information~\cite{MehtaSchwab2012,GovernTenWolde2014} provide the experimental technology to test this bound.
The key test is whether the information efficiency saturates the bound (suggesting the biological system has optimized against it) or falls significantly below it.
...
where $\mathrm{SNR} \sim \mathcal{S}_{\mathrm{tot}}^2\,\theta^4/h_\mu$.
\end{theorem}
...
ratio is
$\mathrm{SNR} \sim \mathcal{S}_{\mathrm{tot}}^2\,\theta^4/h_\mu \ll 1$.
...
The symmetry argument holds exactly for Gaussian innovations and
approximately for symmetric sub-Gaussian innovations.
\end{proof}
...
Here ``freshly calibrated'' means $\theta(t_0) \approx 0$ (not
exactly zero); the linearization $\theta \approx \Lambda \cdot
\Delta t$ is valid in the early-time regime
$\Lambda \cdot \Delta t \ll 1$ of the exponential drift model.
...
\label{T-III-rem:compact_manifold}
For gauge parameters on compact manifolds (e.g.,
$\sigma \in SO(3)$), the van Trees inequality holds provided the
prior is smooth and supported away from coordinate chart
boundaries, which is generically satisfied for the gauge manifolds
considered here.
\end{remark}
...
\item[\textup{(III)}] \textbf{Self-monitoring function.}
  ...
  Theorem~\ref{T-III-thm:loop_stability}.\par}
...
\begin{remark}[Recalibration schedule]
\label{T-III-rem:recalib_schedule}
The bound assumes periodic recalibration at interval
$\tau_{\mathrm{recalib}}$. For threshold-triggered schedules, the
Sivak--Crooks bound applies to each inter-recalibration interval
individually, providing a lower bound on the time-averaged
dissipation rate.
\end{remark}
...
\begin{remark}[Potential double-counting]
\label{T-III-rem:double_counting}
The terms are additive under the assumption that the ego's foreground
processing and the meta-observer's sensing operate on disjoint
physical registers. If these share physical resources, the total may
be an upper bound on the true minimum dissipation.
\end{remark}
...
Asa\.{n}ga, \emph{Mah\=ay\=anasa\d{m}graha (Summary of the Great Vehicle)}, trans.\ \'E.~Lamotte, Peeters (1973).
...
\bibitem{GeKadison1996}
L.~Ge and R.~V.~Kadison, On the structure of the lattice of von Neumann subalgebras, \emph{Proc. Natl. Acad. Sci.} \textbf{93}, 3235 (1996).
...
Vasubandhu, \emph{Tri\d{m}\'{s}ik\=a-vij\~naptim\=atrat\=a (Thirty Verses on Consciousness Only)}, trans.\ S. Anacker, in \emph{Seven Works of Vasubandhu}, Motilal Banarsidass (1984).
...
Zenodo (2026), DOI: 10.5281/zenodo.18400425.
...
S. Liu, \emph{Structural Limits of Unification: Accessibility, Incompleteness, and the Necessity of a Final Cut}, Zenodo (2026), DOI: 10.5281/zenodo.18402907.
...
S.~Liu, \emph{The Realizability Bridge: Algebraic Closure in the Q-RAIF Framework}, Zenodo (2026), DOI: 10.5281/zenodo.18528934.
