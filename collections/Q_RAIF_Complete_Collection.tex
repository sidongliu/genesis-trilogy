...
Lemma~\ref{A-lem:assoc} (associativity as a boundary condition, substantiated by the pathologies of non-associative alternatives),
Lemma~\ref{A-lem:metric} (non-degenerate bilinear form from holographic error correction, connected to the modular Hamiltonian and Fisher information),
and Lemma~\ref{A-lem:signature} (indefinite signature from the algebraic structure of causal order).
...
The physical pathologies of non-associative alternatives are discussed in Remark~\ref{A-rem:nonassoc} of Paper~A.
...
\label{B-rem:scope}
As with Theorem~\ref{A-thm:selection} in Paper~A, the final step of the proof---invoking the universal property of $Cl(V,q)$---is a standard algebraic identity.
The substantive content resides in the constraints:
Lemma~\ref{B-lem:assoc} (associativity as a boundary condition, with non-associative alternatives producing quantifiable pathologies; see Remark~\ref{A-rem:nonassoc}),
and Lemma~\ref{B-lem:metric} (indefinite metric from the requirement of channel discrimination in directed error correction).
The worked example in Section~\ref{B-sec:toymodel} demonstrates that the constraints are simultaneously satisfiable in a concrete Lindblad model.
\end{remark}
...
Moreover, by Lemma~\ref{C-lem:semisimple}, if\/ $\dim V$ is even then $Cl(V,q)$ is simple and $\phi$ is necessarily injective; the image is therefore isomorphic to $Cl(V,q)$ itself, giving a genuine embedding $Cl(V,q) \hookrightarrow \mathcal{A}_{\mathrm{ext}}$.
...
\label{C-rem:scope}
The logical structure of Theorem~\ref{C-thm:closure} is essentially: ``a Clifford algebra, homomorphically embedded in a larger algebra, lands as a Clifford subalgebra.''
...
(iii)~the injectivity guarantee from semisimplicity (Lemma~\ref{C-lem:semisimple}).
